...
## 2026-08-21 Session-first 磨砂 WebUI 与 Agent 运行时开关

- [x] 主界面收敛为浅色磨砂三栏：左侧 Agent 与 Session 导航，中间完整多轮对话，
  右侧固定关联任务详情；审批、原生 Agent 交互、常驻授权和事件流迁入操作中心抽屉；
- [x] Session 支持紧凑下拉筛选；会话消息以用户、Hermes、子 Agent 和工具调用的
  实际发送方、头像及路由展示，不对原始内容做前端总结；纯对话不伪造成任务；
- [x] `agent_controls` 提供管理员运行时开关。停用后心跳仅审计且不能注册或委派；
  重新启用清除旧租约并等待新心跳，避免把过期在线状态误当作可用；
- [x] WebUI 开关 API 受 admin RBAC 和 CSRF 保护；Hermes 解析候选 Agent 与
  State Writer 心跳处理均从同一数据库目标状态读取，防止仅改界面未改调度语义；
- [x] 使用真实持久会话、Hermes/Agent 消息、工具调用和关联任务完成浏览器验收，
  三栏与操作中心均无控制台错误；Agent 开关、调度门禁、心跳门禁和 WebUI API
  已纳入单元回归。
