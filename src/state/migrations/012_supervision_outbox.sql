...
CREATE TABLE supervision_watches (
    id TEXT PRIMARY KEY,
...
CREATE TABLE supervision_outbox (
    id TEXT PRIMARY KEY,
...
CREATE INDEX supervision_watches_peer_status_idx
    ON supervision_watches(peer, status, updated_at);
CREATE INDEX supervision_outbox_delivery_idx
    ON supervision_outbox(status, available_at, lease_until, created_at);
CREATE INDEX supervision_outbox_watch_idx
